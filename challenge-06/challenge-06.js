...
desafio.
*/
var teams = ['Fluminense', 'Vasco da Gama', 'Volta Redonda', 'Madureira', 'Flamengo'];
...
function showTeamPosition(number) {
  return teams[number - 1] ? `O time que está em ${number}º lugar é o ${teams[number - 1]}.`
  : 'Não temos a informação do time que está nessa posição.';
}
...
function convertToHex(cor) {
  ...
  switch (cor) {
    case 'vermelho':
      return `O hexadecimal para a cor ${cor} é ${colors[cor]}.`;
    case 'azul':
      return `O hexadecimal para a cor ${cor} é ${colors[cor]}.`;
    case 'amarelo':
      return `O hexadecimal para a cor ${cor} é ${colors[cor]}.`;
